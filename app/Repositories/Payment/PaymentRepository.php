...
class PaymentRepository implements IPaymentRepository
{
    ...
    {
        $limit = 10;
        return $this->model->where('member_id', $member_id)->where('status', '==', 'paid')->where('equb_id', $equb_id)->with('collecter', 'member', 'equb')->orderBy('created_at', 'DESC')->offset($offset)->limit($limit)->get();
    }
    ////
