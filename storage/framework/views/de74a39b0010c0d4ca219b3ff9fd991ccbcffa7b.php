    ...
    <table id="equb-list-table" class="table table-bordered table-striped">
        ...
        <tbody>
            <?php $__currentLoopData = $member->equbs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $equb): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr id="tre<?php echo e($equb['id']); ?>">
                    <?php
                    // dd($equb);
                   ...
                   ?>
                    <td class="details-control_payment" id="<?php echo e($equb['id']); ?>"></td>
                    <td><?php echo e($key + 1); ?></td>
                    <td>
                        <a href="javascript:void(0);"
                            onclick="openPaymentTab(<?php echo e($equb); ?>)"><?php echo e($equb->equbType->name); ?> round
                            <?php echo e($equb->equbType->round); ?></a>
                    </td>
                    <td> <?php echo e(number_format($equb->amount)); ?></td>
                    <td>
                        <?php
                    ...
                    <td> <?php echo e(number_format($totalPpayment)); ?></td>
                    <td> <?php echo e(number_format($remainingPayment)); ?></td>
                    <td> <?php echo e(number_format($equb->total_amount)); ?></td>
                    ...
                    <td> <?php echo e($finalLotteryInterval != 'passed' ? ($finalLotteryInterval != 'Unassigned' ? $finalLotteryInterval . ' Days' : 'Unassigned') : 'Passed'); ?>

                    </td>
                    <td> <?php echo e($endDateInterval != 'passed' ? $endDateInterval . ' Days' : 'Passed'); ?></td>
                    <td> <?php echo e($equb->status); ?></td>
                    <?php
                            ...
                            <div class='dropdown'>
                                ...
                                <ul class='dropdown-menu p-4'>
                                    ...
                                    <?php if(Auth::user()->role != 'finance'): ?>
                                        <li>
                                            <a href="javascript:void(0);"
                                                class="text-secondary btn btn-flat <?php echo e($member->status == 'Deactive' ? 'disabled' : ($equb->status == 'Deactive' ? 'disabled' : ($sum >= $expectedTotal ? 'disabled' : ''))); ?>"
                                                onclick="openPaymentModal(<?php echo e($equb); ?>)" id="paymentButton"><i
                                                    class="fas fa-plus-circle"></i> Payment</a>
                                        </li>
                                        <li>
                                            <a href="javascript:void(0);"
                                                class="text-secondary btn btn-flat <?php echo e($member->status == 'Deactive' ? 'disabled' : ($equb->status == 'Deactive' ? 'disabled' : ($remainingAmount == 0 ? 'disabled' : ''))); ?>"
                                                id="lotteryPaymentButton"
                                                onclick="openLotteryModal(<?php echo e($equb); ?>)"><i
                                                    class="fas fa-plus-circle"></i> Lottery</a>
                                        </li>
                                        <li>
                                            <a href="javascript:void(0);"
                                                class="text-secondary btn btn-flat <?php echo e($member->status == 'Deactive' ? 'disabled' : ($equb->status == 'Deactive' ? 'disabled' : ($remainingAmount == 0 && $sum == $expectedTotal ? 'disabled' : ''))); ?>"
                                                onclick="openEqubEditModal(<?php echo e($equb); ?>)"
                                        ...
                                        <li>
                                            <a href="javascript:void(0);" class="text-secondary btn btn-flat"
                                                onclick="equbDrawCheckChange(<?php echo e($equb); ?>)"
    ...
    <script type="text/javascript">
        function equbStatusChange(item) {
            $('#aaaa').modal('show');
            $('#bbbb').attr('action', "<?php echo e(url('member/equbStatus-update')); ?>" + '/' + item.id);
        }

        function equbDrawCheckChange(item) {
            $('#checkForEqubModal').modal('show');
            $('#checkForDrawForm').attr('action', "<?php echo e(url('member/equb-check-for-draw-update')); ?>" + '/' + item.id);
        }
        ...
        function addUnpaid(item) {
            $.ajax({
                url: "<?php echo e(url('member/add-unpaid')); ?>" + '/' + item.id,
                headers: {
            ...
            $('#addUnpaidButton').attr('action', "<?php echo e(url('member/add-unpaid')); ?>" + '/' + item.id);
        }
        $(function() {
            ...
            $('#equb-list-table tbody').on('click', 'td.details-control_payment', function() {
                ...
                if (row.child.isShown()) {
                    ...
                    $.ajax({
                        url: "<?php echo e(url('member/show-equb')); ?>" + '/' + inputId,
                        type: 'get',
                        success: function(data) {
                            row.child(data).show();
                            row.child.show();
...
<?php /**PATH D:\virtual Equb\virtual-backend\resources\views/admin/member/memberDetails.blade.php ENDPATH**/ ?>