...
@if (Auth::user()->role == 'admin' ||
    ...
    <div class="row justify-content-center">
        <p class="card-title">Details on {{ $type }} logs</p>
    </div>
            ...
            <ul class="pagination">
                ...
                @else
                    <li class="page-item">
                        <a class="page-link" href="javascript:void(0);"
                            onclick="viewSearchLogsPaginate('{{ $searchInput }}',{{ $offset - $limit }},{{ $pageNumber - 1 }})"
                            aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                            <span class="sr-only">Previous</span>
                        </a>
                ...
                @if ($pageNumber > 3)
                    <li class="page-item"><a class="page-link" href="javascript:void(0);"
                            onclick="viewSearchLogsPaginate('{{ $searchInput }}',{{ $offset - $limit * 3 }},{{ $pageNumber - 3 }})">{{ $pageNumber - 3 }}</a>
                    </li>
                @endif
                @if ($pageNumber > 2)
                    <li class="page-item"><a class="page-link" href="javascript:void(0);"
                            onclick="viewSearchLogsPaginate('{{ $searchInput }}',{{ $offset - $limit * 2 }},{{ $pageNumber - 2 }})">{{ $pageNumber - 2 }}</a>
                    </li>
                @endif
                @if ($pageNumber > 1)
                    <li class="page-item"><a class="page-link" href="javascript:void(0);"
                            onclick="viewSearchLogsPaginate('{{ $searchInput }}',{{ $offset - $limit }},{{ $pageNumber - 1 }})">{{ $pageNumber - 1 }}</a>
                    </li>
                @endif
